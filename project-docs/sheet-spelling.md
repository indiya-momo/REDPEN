...
| find  | replace | enabled | tip     | memo | counts_in_quota | visible | divider_group | divider_label  | overlay_replace | rule_id      | finds | display_label |
| ----- | ------- | ------- | ------- | ---- | --------------- | ------- | ------------- | -------------- | --------------- | ------------ | ----- | ------------- |
| 우리 나라 | 우리나라    | TRUE    | 띄어쓰기 통일 |      | TRUE            | TRUE    |               |                |                 |              |       |               |
| 애덤    | 아담      | FALSE   | 외래어표기법… |      | TRUE            | TRUE    | C             | 잘못된 외래어 표기(명사) | →아담             | foreign-adam | 애덤,애썸 | 애덤·애썸 → 아담    |
...
- **visible**: `TRUE`(기본) / `FALSE` — `FALSE`면 **왼쪽 규칙 목록에만 숨김**. `enabled=TRUE`이면 검사에는 포함됨(시트에 쌓아 두고 사용자에게 안 보이게 할 때)
- **divider_group** · **divider_label** · **overlay_replace**: 묶음·PDF 오버레이 (기존과 동일)
- **rule_id**: (선택) 논리 규칙 고정 키 — 체크 on/off·프로젝트 저장에 사용. `finds`와 함께 쓰면 권장
- **finds**: (선택) **쉼표**로 구분한 잘못된 표기 이형태. **2개 이상**일 때만 한 행·한 체크로 묶임 (`애덤,애썸` → 모두 `replace`로 교정)
- **display_label**: (선택) 체크·결과에 보일 이름. 비우면 `애덤·애썸 → 아담` 형식으로 자동 생성
...
### 이형태 묶음 (`finds`) vs 공백 bulk


| 방식             | 시트 예                                 | sync 결과          | 용도                            |
| -------------- | ------------------------------------ | ---------------- | ----------------------------- |
| **finds (쉼표)** | `find=애덤` `replace=아담` `finds=애덤,애썸` | **규칙 1개**, 체크 1줄 | 같은 교정어의 이형태                   |
| **공백 bulk**    | `find=갯수 싯가` `replace=개수 시가`         | **규칙 2개**, 체크 2줄 | 서로 다른 find↔replace 쌍을 한 행에 입력 |
...
1. 시트 **공유**: 「링크가 있는 모든 사용자」**보기** 이상 (CSV export용)
...
| `src/lib/builtInRules.js`        | JSON → BUILT_IN_RULES · 체크 키(rule_id/find) |
| `src/lib/spellingRuleEntry.js`   | finds 파싱 · 검사용 규칙 빌드                       |
