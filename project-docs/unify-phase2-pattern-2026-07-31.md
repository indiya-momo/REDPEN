...
**상태:** 1차 플로우 구현됨 · **P0 추천 엔진(증거 하한·Score·support·Preview) 구현** · P0.5~P2 미구현 · **제품 목표 3단계는 로드맵 확정(아래 §0), 3단 미구현**  
**날짜:** 2026-07-31  
**개정:** 2026-08-01 — Discover는 유지, 패턴 단계를 추천 엔진으로 보강하는 우선순위 합의 · P0 구현  
**개정:** 2026-08-06 — 제품 목표 **3단계**(철자·표기 이형태)를 로드맵에 명시  
**관련:** `unify-pattern-rule-session-2026-07-31.md` (초기 세션 confirm 안 — **본 문서로 대체**) · `unify-candidate-spacing-redesign-2026-07-29.md` §0

## §0 — 제품 목표 단계 (로드맵)

표기 통일 **추천**이 겨냥하는 큰 단계는 셋이다. (아래 UX의 「1차/2차 플로우」와 숫자를 혼동하지 말 것.)

| 목표 단계 | 내용 | 예 | 상태 |
|-----------|------|----|------|
| **1. 띄어쓰기 기반** | 같은 키·띄움/붙임만 다른 이형태를 찾아 통일 제안 | `금융위기` ↔ `금융 위기` | **구현·운용** (Discover + 본 문서 1차 플로우) |
| **2. 요소(편측) 통일** | 1에서 확정한 정책을 **접두·접미 등 요소**로 확장 | `경제○○` 붙임 확정 → 같은 접두 계열 제안 | **구현·운용** (본 문서 2차 pattern) |
| **3. 철자·표기 이형태 통일** | 띄어쓰기와 무관한 **철자·음차·표기** 이형태를 문서 안에서 묶어 통일 제안 | `도널드` ↔ `도날드`, 유사 고유명·외래 음차 | **해야 함 · 스파이크 중** (`unify-stage3-orthography-spike-2026-08-06.md`) |

- 3단계는 맞춤법 시트(`find`→`replace` 규범 교정)와 **겹칠 수 있으나 직무가 다름**: 시트는 “틀린 표기→표준”, 3단계는 “원고 안 **혼용**을 한쪽으로 맞춤”(어느 쪽이 표준인지는 다수형·편집자 선택·선택적 규범 힌트).  
- 3단 이전에는 수동 「표기 통일하기」등록으로 비띄어쓰기 이형태를 보완한다. 3단이 붙으면 추천 경로로 올리고, 수동은 예외·미리 등록용으로 유지.
- 스파이크 1차: **라틴 괄호 병기 앵커** → 다수형 추천 · `needsVerification`. Discover/UI 미연결.

## 의도 (한 줄)

목표 1·2에 대해: 1차(띄어쓰기)는 검수권으로 산 **본작업**이라 목록 완결을 유도하고, 2차(요소)는 확정 정책을 **편측(접두·접미)까지 확장**하는 **부가 단계**라 미선택·중도 종료를 허용한다.  
Discover(발견)는 이미 성숙했고, 다음 체감 개선은 **Score → Explain(메타) → Preview**로 “왜 추천하는지 / 적용하면 무엇이 바뀌는지”를 보여주는 것이다. **목표 3(철자 이형태)은 별도 Discover·클러스터 설계가 필요하다.**
