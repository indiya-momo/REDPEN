...
describe('auxiliaryVerbPattern', () => {
  ...
  it('해 지 — stem (에서 지켜 오탐 없음)', () => {
    ...
    expect(rules.some((r) => matches(r, '상상해 지다'))).toBe(true);
  ...
  it('해 보·해 가 — 통해+명사(관형어) 오탐 없음', () => {
    ...
    expect(matches(bo, '상상해 보자')).toBe(true);
    expect(matches(ga, '먹어 가')).toBe(false);
    expect(matches(ga, '상상해 가')).toBe(true);
  });
  ...
  it('해 보 — 띄움만', () => {
    ...
    expect(rules.length).toBe(1);
    expect(matches(rules[0], '상상해 보자')).toBe(true);
    expect(matches(rules[0], '상상해보자')).toBe(false);
    expect(matches(rules[0], '통해보장')).toBe(false);
  });

  it('해보 — 해 보와 동일, 띄움만', () => {
    const rules = buildAuxiliaryVerbFindRules('해보');
    expect(rules.length).toBe(1);
    expect(matches(rules[0], '상상해 보아요')).toBe(true);
    expect(matches(rules[0], '상상해보아요')).toBe(false);
    expect(matches(rules[0], '해보세요')).toBe(false);
  });
});
